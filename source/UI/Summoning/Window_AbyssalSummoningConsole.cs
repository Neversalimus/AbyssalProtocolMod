    ...
    public class Window_AbyssalSummoningConsole : Window
    {
        ...
        private sealed class Window_AbyssalCircleInfrastructure : Window
        {
            ...
            private void DoWindowContentsSafe(Rect inRect)
            {
                ...
                float gap = 8f;
                Rect summaryRect = new Rect(inner.x, inner.y, inner.width, 52f);
                Rect capacitorRect = new Rect(inner.x, summaryRect.yMax + gap, inner.width, 122f);
                Rect stabilizerRect = new Rect(inner.x, capacitorRect.yMax + gap, inner.width, 214f);
                Rect effectRect = new Rect(inner.x, stabilizerRect.yMax + gap, inner.width, Mathf.Max(100f, inner.yMax - stabilizerRect.yMax - gap));
        ...
        private void DrawCompactInfrastructureEffectSection(Rect rect, AbyssalSummoningConsoleUtility.RitualDefinition ritual)
        {
            ...
            string tooltip = AbyssalSummoningConsoleUtility.TranslateOrFallback(
                ...
                "Shows how the installed capacitor lattice and stabilizer ring are supporting the selected ritual. Use this window to install, replace, or remove circle slot modules.");
            TooltipHandler.TipRegion(rect, tooltip);
        }
