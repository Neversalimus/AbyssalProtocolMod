...
| Forge implant subfilters using only broad text matching | P2 | Forge UI / implant recipes | Brain, optic/sight, stomach, or other slot-specific implants fall into the Body subfilter because keywords are missing or description text dominates matching | Prefer identity-based overrides for known implants before broad search text; keep slot keywords such as `stomach`, `optic`, `subnode` in the resolver; audit implant subfilters after adding new implant families. |
| Forge Melee/Ranged filter inferred from display text | P1 | Forge UI / weapon patterns | A real melee weapon whose name lacks `blade`, `dagger`, `maul`, `glaive`, or another legacy keyword is listed under Ranged | Classify the produced `ThingDef` first: `IsRangedWeapon` is authoritative for ranged weapons; non-ranged weapon defs with inherited melee tools are melee. Keep text only as a fallback for incomplete legacy recipes. Regression-test Dominion Breach Driver and Crown Scission Array whenever the Forge filter logic changes. |
...
## UI regression note — compact infrastructure window clipping
...
## UI state leakage and wrapper cleanup guards — 2026-05-24

| Risk | Severity | Area | Symptoms | Prevention / check |
| --- | --- | --- | --- | --- |
| Vanilla scroll views close correctly but leave dirty GUI/Text state after draw exceptions | P3 | settings/diagnostics UI | Following UI panels inherit wrong text font, anchor, or color after a recoverable draw exception | Pair `Widgets.EndScrollView()` with `ABY_UISafetyUtility.ResetTextAndGUIState()` in `finally` blocks, not only custom Abyssal scroll wrappers. |
