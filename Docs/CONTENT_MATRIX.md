...
| Abyssal Forge UI | Implemented | `source/UI/Forge/`, `source/Forge/` | `Defs/ThingDefs/`, `Defs/RecipeDefs/`, `Defs/ResearchProjectDefs/` | `Textures/UI/**`, forge building/item textures | Forge compact ITab + full Forge/Communion Console | residue, unlocks, recipes, boss drops, explicit Protocol Nexus gates | New forge content should normally surface in custom Forge UI. For Protocol-gated Forge patterns, set `requiredProtocolResearchDefName` on the RecipeDef extension or product ThingDef extension; do not rely on fallback inference for key current content. |
...
| Protocol Nexus / custom research | Experimental / Active | `source/Experimental/ProtocolResearch/`, `source/Progression/` | `Defs/Experimental/ProtocolResearch/`, `Defs/ResearchProjectDefs/` | `Textures/UI/ABY/ProtocolResearch/` | Protocol Nexus UI | protocol segments, research gates, forge/summon relation | Treat as real active surface but expect layout/visual iteration. Current Forge unlock extensions should prefer explicit `requiredProtocolResearchDefName` links over name/residue auto-inference when the pattern is important or implemented. |
...
## Protocol Nexus authority checklist

When adding or retuning Forge-visible content:

```text
1. Add or verify DefModExtension_AbyssalForgeUnlock on the RecipeDef or primary product ThingDef.
2. Set requiredResidue and category as before.
3. Set requiredProtocolResearchDefName explicitly for important/current content.
4. Prefer active, decodable Protocol Research projects for implemented recipes.
5. Do not point current playable Forge content at futureReserve Protocol projects unless the item is intentionally unreleased/hidden.
6. Keep material costs, boss-drop ingredients, Summoning kill-gates, and Protocol decode requirements aligned so the UI promise matches the actual unlock.
```
