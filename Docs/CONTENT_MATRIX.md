...
## Apparel balance baseline — 2026-05-21
| System | Status | Source | Defs | UI | Progression | Notes |
|---|---|---|---|---|---|---|
| Apparel armor baseline / armor-aegis / hover armor | Implemented / needs smoke test | `source/Apparel/`, `source/Apparel/Comps/`, `source/Apparel/Stats/` | `Defs/ThingDefs/ABY_*Armor*.xml`, `Defs/ThingDefs/ABY_*Apparel*.xml`, `Defs/RecipeDefs/ABY_*Apparel*_Recipes.xml`, `Defs/RecipeDefs/ABY_*Armor*_Recipes.xml`, `Patches/ABY_ApparelAegis_ArmorPatches.xml` | apparel info cards, Aegis gizmo/status | Forge armor recipes/unlocks | T1 combat armor is now balanced around vanilla endgame. Saint/Crown torso armor use armor-aegis profiles. Hover speed bonuses are controlled passive bonuses, not +3.0 mobility breaks. Positive shooting/melee/dodge offsets on apparel should remain at least 0.10 to avoid stat-card rounding to 0.0. |
