...
| TPS optimized hot loops | Implemented / ongoing | `source/Compatibility/`, `source/Apparel/`, `source/Combat/`, `source/Bosses/Rupture/`, `source/Comps/`, `source/Dominion/` | mass enemy scan intervals in `Defs/ThingDefs/` | Forge/Summoning/shared UI should respect reduced animation | Future combat/Dominion additions should be checked against this matrix before adding new timers; recurring combat comps should prefer `ABY_RuntimeTargetCache` over direct map pawn scans. |
