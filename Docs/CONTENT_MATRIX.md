...
| Boss Bar / phase UI / Aegis display / miniboss HP bars | Implemented / Fragile | `source/UI/BossBar/`, `source/Bosses/Shared/`, `source/Bosses/ReactorSaint/`, `source/Core/Utilities/ABY_AbyssalPawnClassificationUtility.cs` | `Defs/Misc/ABY_BossBarProfiles.xml`, `Defs/PawnKindDefs/` | `Textures/UI/ABY/BossBar/` | Boss Bar UI plus overhead miniboss bars | boss/miniboss state | High regression risk: clipping, phase display, chain/aegis placement, dynamic HP fill. Minibosses with `CompABY_BossTrueDeath` should use compact overhead bars instead of the full cinematic boss HUD unless deliberately promoted to a major boss. The active draw call belongs in `AbyssalBossScreenFXGameComponent` so existing saves receive the UI without requiring a newly-added GameComponent instance. |
