...
| Sigil invocation transaction and abort recovery | `ABY_SigilInvocationTransaction`, `CompUseEffect_SummonBoss`, `Building_AbyssalSummoningCircle`, `JobDriver_CarrySigilToAbyssalCircle` | Normal sigil use, Console, manual use, and Sigil Vault | Sigil is permanently spent only after concrete encounter activation. Pre-activation abort/failure refunds one sigil, held-sigil warmup reports a precise reason, and player-facing pre-activation abort is available from the Circle/Console. |
| Multi-record summon runtime lifecycle | `MapComponent_ABY_SummonEncounterRuntime` | Map runtime / Dev rehearsal diagnostics | Normal player paths retain one active encounter. Confirmed Dev concurrent rehearsal retains separate lifecycle records instead of overwriting the previous test encounter. |
| Explicit early capacitor exemption policy | `AbyssalCircleCapacitorRitualUtility` | Summoning Console / preflight | `unstable_breach`, `ember_hunt`, and `archon_beast` intentionally have no capacitor profile. This is explicit policy; compatibility code does not mutate private static profiles through reflection. |
