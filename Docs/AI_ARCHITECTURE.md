...
The normal carrier path is `JobDriver_CarrySigilToAbyssalCircle`: acquire the sigil, then move to the interaction cell, hold throughout warmup, and activate. It must route all readiness checks through `ABY_SigilUseValidator` and `ABY_SummonPreflightReport`; do not reintroduce independent ground staging rules. Acquisition validation may require that the pawn carries the sigil, but it must not enforce the exact interaction-cell position until after the movement toil completes; priming, warmup, and final activation must enforce that positional invariant.
