...
namespace AbyssalProtocol
{
    ...
    public static class DominionSliceVfxUtility
    {
        ...
        public static void DrawAnchorLink(Vector3 anchorPos, Vector3 heartPos, Map map, DominionSliceAnchorRole role, int seed)
        {
            ...
            DrawBeam(anchorPos, heartPos, width * 2.75f, length, LinkBeamMaterial, breath);
            DrawBeam(anchorPos, heartPos, width * 0.82f, length, LinkCoreMaterial, 1f + (pulse - 1f) * 0.55f);
        }
        ...
        public static void SpawnHeartExposedBurst(Vector3 position, Map map)
        {
            ...
            if (moteDef != null)
            {
                MoteMaker.MakeStaticMote(position, map, moteDef, 3.15f);
                MoteMaker.MakeStaticMote(position + new Vector3(0f, 0.004f, 0f), map, moteDef, 2.10f);
            }

            FleckMaker.ThrowLightningGlow(position, map, 3.4f);
            FleckMaker.ThrowMicroSparks(position, map);
            FleckMaker.ThrowMicroSparks(position, map);
            FleckMaker.ThrowMicroSparks(position, map);
