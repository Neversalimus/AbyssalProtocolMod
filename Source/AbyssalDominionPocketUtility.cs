...
public static bool TryOpenPocketSlice(Building_AbyssalDominionGate gate, IEnumerable<Pawn> pawns, out ABY_DominionPocketSession session, out string failReason)
        ...
        public static int GetPocketPlayerCount(Map map)
        {
            return map == null ? 0 : GetPocketPlayerPawns(map).Count;
        }
        ...
        public static string GetPocketSessionStatusValue(ABY_DominionPocketSession session, Map pocketMap)
        {
            ...
            if (!session.lastOutcomeReason.NullOrEmpty() && !session.active)
            {
            ...
            if (session.victoryAchieved)
            {
                string collapseEta = encounter != null ? encounter.GetCollapseEta() : "ABY_DominionSliceEncounter_CollapseInactive".Translate();
        ...
        public static string GetPocketObjectiveValue(ABY_DominionPocketSession session, Map pocketMap)
        {
            ...
            if (!session.lastOutcomeReason.NullOrEmpty())
            {
                return session.lastOutcomeReason;
            }
        ...
        public static string GetPocketEncounterTelemetry(ABY_DominionPocketSession session, Map pocketMap)
        {
            ...
            string statusValue = GetPocketSessionStatusValue(session, pocketMap);
            string objectiveValue = GetPocketObjectiveValue(session, pocketMap);
            string rewardValue = GetPocketRewardValue(session, pocketMap);
            return "ABY_DominionPocketTelemetry_Report".Translate(statusValue, objectiveValue, rewardValue);
        }
