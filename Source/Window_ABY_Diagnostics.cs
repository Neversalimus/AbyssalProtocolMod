...
{
    public sealed class Window_ABY_Diagnostics : Window
    {
        ...
        {
            Find.WindowStack?.Add(new Window_ABY_Diagnostics());
        }

        public override void DoWindowContents(Rect inRect)
        {
            ABY_UISafetyUtility.TryDo("Abyssal diagnostics window", delegate
            {
                Text.Font = GameFont.Medium;
                ABY_UIPolishUtility.Label(new Rect(inRect.x, inRect.y, inRect.width, 34f), "Abyssal Protocol diagnostics", TextAnchor.MiddleLeft, GameFont.Medium);
                ...
                {
                    GUIUtility.systemCopyBuffer = ABY_StabilityDiagnosticsUtility.BuildPlainTextReport();
                    Messages.Message("Abyssal diagnostics copied to clipboard.", MessageTypeDefOf.NeutralEvent, false);
                }
        ...
        {
            float total = 8f;
            List<string> lines = cachedLines ?? ABY_StabilityDiagnosticsUtility.BuildStatusLines();
